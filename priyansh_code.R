...
model_12_states <- model_results[[which(state_counts == 6)]]$model

# using getpars() to extract the parameters from the trained model
parameters <- getpars(model_12_states)
...
test_results <- evaluate_hmm_test(trained_model = model_12_states,
...
group_logLik_results <- mapply(
  ...
  MoreArgs = list(trained_model = model_12_states),
  SIMPLIFY = FALSE
)
